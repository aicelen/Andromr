...
from backend.custom_scipy.peaks import find_peaks

from backend import layers
from backend.bbox import BBox, find_lines, get_bbox, get_center
...
class Line:
    ...
    def slope(self) -> float:
        ...
        #linear regressiom with np
        coef = np.polyfit(xs,ys,1)
        poly1d_fn = np.poly1d(coef) 
        print(f"SLOPE calc numpy: {poly1d_fn[1]}")
