...
from backend.symbol_extraction import Barline, Clef, Sfn, Rest, SfnType, ClefType, RestType
from backend.note_group_extraction import NoteGroup
from backend.notehead_extraction import NoteHead, NoteType
from backend.utils import get_global_unit_size, get_total_track_nums
...
class Measure:
    ...
    def get_key(self) -> Key:
        ...
        if not all_equal:
            print("The number of key symbols are not all the same for every track!")
            ...
            if pos_dura != 0:
                ...
                for sym in time_slots[add_idx]:
                    if sym.track == 1-lead_track:
                        ...
                        print(f"Before: {ori_label}, Dot: {ori_dot}" \
                ...
                added += 1
                print(f"Add: {diff}, {rest}, {insert_idx}, {add_idx}, {1-lead_track}")
...
class MusicXMLBuilder:
    ...
    def build(self) -> None:
        ...
        try:
            ...
            first_measure = self.measures[1][0]
