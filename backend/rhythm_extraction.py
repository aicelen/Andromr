...
from backend.utils import get_unit_size
from backend.bbox import BBox, get_center, get_rotated_bbox, to_rgb_img, draw_bounding_boxes
from backend.notehead_extraction import NoteType
from backend.morph import morph_open, morph_close
...
def parse_beam_overlap_regions(poly_map: ndarray, invalid_map: ndarray) -> Tuple[ndarray, Dict[int, Dict[str, Any]]]:
    ...
    mix[mix>1] = 1
    reg_map, feat_num = label_f(poly_map)
    sym_map, _ = label_f(mix)
