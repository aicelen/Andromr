...
from backend.utils import find_closest_staffs, get_global_unit_size, get_unit_size
...
def predict_symbols():
    ...
    idx = np.where(pred>0)
    img[idx[0], idx[1]] = 0
    tf_model = PredictSymbols("sfn")
    for box in bboxes:
        region = pred[box[1]:box[3], box[0]:box[2]]
        region[region>0] = 255
        pp = tf_model.predict(region)
