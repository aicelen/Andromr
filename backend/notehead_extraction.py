...
from backend.constant import NoteHeadConstant as nhc
from backend.bbox import BBox, get_bbox, get_center, merge_nearby_bbox, rm_merge_overlap_bbox, to_rgb_img
from backend.utils import get_unit_size, find_closest_staffs, get_global_unit_size
from backend.staffline_extraction import Staff
...
class NoteHead:
    ...
    def label(self) -> Union[NoteType, None]:
        if self.invalid:
            print(f"Note {self.id} is not a valid note.")
            return None
        return self._label
...
def get_notehead_bbox(
    ...
    print("Detected noteheads: %d", len(result_bboxes))

    print("Filtering noteheads")
    ...
    print("Detected noteheads after filtering: %d", len(bboxes))
    return bboxes
...
def parse_stem_info(notes: List[NoteHead]) -> None:
    ...
    st_map, _ = label_f(enhanced_stems)
...
def extract(
    ...
    print("Analyzing notehead bboxes")
