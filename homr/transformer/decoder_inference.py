...
class ScoreDecoder:
    ...
    def generate(
        ...
        for step in range(self.max_seq_len):
            ...
            lift_token = detokenize(liftsp, self.inv_lift_vocab)
            pitch_token = detokenize(pitchsp, self.inv_pitch_vocab)
            rhythm_token = detokenize(rhythmsp, self.inv_rhythm_vocab)
            articulation_token = detokenize(articulationsp, self.inv_articulation_vocab)
            slur_token = detokenize(slursp, self.inv_slur_vocab)
            position_token = detokenize(positionsp, self.inv_position_vocab)

            if rhythmsp.item() == self.eos_token:
                break
            ...
            out_lift[0, 0] = liftsp.item()
            out_pitch[0, 0] = pitchsp.item()
            out_rhythm[0, 0] = rhythmsp.item()
            out_articulations[0, 0] = articulationsp.item()
            out_slurs[0, 0] = slursp.item()
        return symbols
